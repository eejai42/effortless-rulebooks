...
Scanner-generated findings (cr-17-*, cr-19-slot-*) are re-derived by
scripts/scan-project-slots.py; use this only for findings whose rule has no
mechanical witness. Refuses unknown ids and ids that are already fixed.
...
def main() -> None:
    ...
    for fid in ids:
        ...
        if fid.startswith("cr-19-slot-") or fid.startswith("cr-17-"):
            raise SystemExit(f"{fid!r} is scanner-derived; re-run scripts/scan-project-slots.py instead")
