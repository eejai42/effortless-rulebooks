...
PROJECT_NAME='is-everything-a-language'
EXPERIENCE_DESCRIPTION='Language-classification board reading the rulebook views'
...
export PGDATABASE="${PGDATABASE:-erb_is_everything_a_language}"
...
for file in app/package.json app/server.js app/vite.config.js \
  effortless-rulebook/is-everything-a-language-rulebook.json; do
...
view_ready="$(psql -d "$PGDATABASE" -v ON_ERROR_STOP=1 -tAc \
  ...
  2>/dev/null || true)"
[ "$view_ready" = "1" ] \
  || die "database $PGDATABASE at $PGHOST:$PGPORT is unavailable or has no vw_* views; load the committed postgres-bootstrap artifacts first"
