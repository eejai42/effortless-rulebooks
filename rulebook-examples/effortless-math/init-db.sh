#!/usr/bin/env bash
# Load this project's local database from the generated effortless-postgres/ output.
# The generated init-db.sh defaults to the wrong database name, so the URL is fixed here.
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
INIT_SCRIPT="$SCRIPT_DIR/effortless-postgres/init-db.sh"
DATABASE_NAME="erb_effortless_math"
