...
| Scale Ratio | Computed as the scale divided by the NULLIF of the base scale and 0. | _How many times larger than base scale (multiplicative factor)._ |
| Log Scale Normalized | Computed as the log scale minus the system min log scale divided by the NULLIF of the system delta log scale and 0. | _Position within system's log-scale range as 0-1 value._ |
...
| Empirical Log Log Slope | Computed as the delta log measure divided by the NULLIF of the delta log scale and 0. | _Calculated: Slope of log-log line from empirical data._ |
...
| Slope to Noise Ratio | Computed as the absolute value of the empirical log log slope divided by the NULLIF of the noise sigma and 0. | _Signal-to-noise ratio (slope magnitude vs. noise level)._ |
...
| Data Density | Computed as the point count divided by the NULLIF of the log log area and 0. | _Number of data points per unit area in log-log space._ |
| Relative Slope Error | Computed as the slope error divided by the NULLIF of the theoretical log log slope and 0. | _Slope error as fraction of theoretical slope (percentage)._ |
...
| Outlier Rate | Computed as the outlier count divided by the NULLIF of the total point count and 0. | _Fraction of points that are outliers (quality metric)._ |
...
| Standardized Residual | Computed as the residual divided by the NULLIF of the residual RMS and 0. | _Residual in units of RMS (number of standard deviations)._ |
...
| Scale Ratio | Computed as the scale divided by the NULLIF of the base scale and 0. | _Scale relative to base scale (multiplicative factor)._ |
...
| Slope Confidence Interval | Computed as 1.96 times the residual RMS divided by the NULLIF of the square root of the point count and 0. | _Approximate 95% confidence interval for slope (simplified formula)._ |
...
| Fit Efficiency | Computed as the r2 divided by the NULLIF of the one plus residual RMS and 0. | _Quality per unit error (higher is better)._ |
| Normalized RMSE | Computed as the residual RMS divided by the NULLIF of the log measure range and 0. | _RMSE as fraction of total log measure range._ |
| Slope to Theoretical Ratio | Computed as the fitted slope divided by the NULLIF of the theoretical log log slope and 0. | _Ratio of fitted to theoretical slope (1.0 = perfect match)._ |
...
| Adjusted R2 | Computed as 1 minus the one minus r2 times the point count minus one divided by the NULLIF of the point count minus two and 0. | _R² adjusted for degrees of freedom._ |
...
| **DR-20 Scale Ratio** | A scale's scale ratio is computed as the scale divided by the NULLIF of the base scale and 0. |
| **DR-21 Log Scale Normalized** | A scale's log scale normalized is computed as the log scale minus the system min log scale divided by the NULLIF of the system delta log scale and 0. |
...
| **DR-31 Empirical Log Log Slope** | A system stat's empirical log log slope is computed as the delta log measure divided by the NULLIF of the delta log scale and 0. |
...
| **DR-39 Slope to Noise Ratio** | A system stat's slope to noise ratio is computed as the absolute value of the empirical log log slope divided by the NULLIF of the noise sigma and 0. |
...
| **DR-43 Data Density** | A system stat's data density is computed as the point count divided by the NULLIF of the log log area and 0. |
| **DR-44 Relative Slope Error** | A system stat's relative slope error is computed as the slope error divided by the NULLIF of the theoretical log log slope and 0. |
...
| **DR-48 Outlier Rate** | A measurement model's outlier rate is computed as the outlier count divided by the NULLIF of the total point count and 0. |
...
| **DR-66 Standardized Residual** | An observed scale's standardized residual is computed as the residual divided by the NULLIF of the residual RMS and 0. |
...
| **DR-68 Scale Ratio** | An observed scale's scale ratio is computed as the scale divided by the NULLIF of the base scale and 0. |
| **DR-69 Abs Residual** | An observed scale's abs residual is computed as the absolute value of the residual. |
| **DR-70 Slope Confidence Interval** | An inference run's slope confidence interval is computed as 1.96 times the residual RMS divided by the NULLIF of the square root of the point count and 0. |
...
| **DR-79 Fit Efficiency** | An inference run's fit efficiency is computed as the r2 divided by the NULLIF of the one plus residual RMS and 0. |
| **DR-80 Normalized RMSE** | An inference run's normalized RMSE is computed as the residual RMS divided by the NULLIF of the log measure range and 0. |
| **DR-81 Slope to Theoretical Ratio** | An inference run's slope to theoretical ratio is computed as the fitted slope divided by the NULLIF of the theoretical log log slope and 0. |
...
| **DR-85 Adjusted R2** | An inference run's adjusted r2 is computed as 1 minus the one minus r2 times the point count minus one divided by the NULLIF of the point count minus two and 0. |
...
| **scales.ScaleRatio** | formula | `Scale / Nullif(BaseScale, 0)` |
| **scales.LogScaleNormalized** | formula | `LogScale - SystemMinLogScale / Nullif(SystemDeltaLogScale, 0)` |
...
| **system_stats.EmpiricalLogLogSlope** | formula | `DeltaLogMeasure / Nullif(DeltaLogScale, 0)` |
...
| **system_stats.SlopeToNoiseRatio** | formula | `Abs(EmpiricalLogLogSlope) / Nullif(NoiseSigma, 0)` |
...
| **system_stats.DataDensity** | formula | `PointCount / Nullif(LogLogArea, 0)` |
| **system_stats.RelativeSlopeError** | formula | `SlopeError / Nullif(TheoreticalLogLogSlope, 0)` |
...
| **measurement_models.OutlierRate** | formula | `OutlierCount / Nullif(TotalPointCount, 0)` |
...
| **observed_scales.StandardizedResidual** | formula | `Residual / Nullif(ResidualRMS, 0)` |
| **observed_scales.IsOutlier** | formula | `Abs(StandardizedResidual) > 2.5` |
| **observed_scales.ScaleRatio** | formula | `Scale / Nullif(BaseScale, 0)` |
| **observed_scales.AbsResidual** | formula | `Abs(Residual)` |
| **inference_runs.SlopeConfidenceInterval** | formula | `1.96 * ResidualRMS / Nullif(Sqrt(PointCount), 0)` |
...
| **inference_runs.FitEfficiency** | formula | `R2 / Nullif(OnePlusResidualRMS, 0)` |
| **inference_runs.NormalizedRMSE** | formula | `ResidualRMS / Nullif(LogMeasureRange, 0)` |
| **inference_runs.SlopeToTheoreticalRatio** | formula | `FittedSlope / Nullif(TheoreticalLogLogSlope, 0)` |
...
| **inference_runs.AdjustedR2** | formula | `1 - OneMinusR2 * PointCountMinusOne / Nullif(PointCountMinusTwo, 0)` |
