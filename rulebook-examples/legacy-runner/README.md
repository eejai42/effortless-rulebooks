...
**What this is.** A transitional retirement staging area for the former root and its `effortless-platform/` folder. It still contains the admin portal (`:7777`), `ssotme-proxy` bus (`:4242`), execution substrates, and conformance harness, but it is not the future platform or a showcase to extend. Under the root [Platform Explorer and Repository Consistency Plan](../../PLATFORM-EXPLORER-PLAN.md), each capability must be promoted, separated, replaced, or consciously retired before removal. The historical documents below preserve the old operating model; they do not override the retirement plan.

```bash
cd rulebook-examples/legacy-runner
./start.sh              # ssotme-proxy on :4242 + admin portal on :7777 (kills stale listeners first)
ERB_DOMAIN=<slug> ./start.sh --cli   # the orchestration menu for one project
effortless build        # regenerate postgres/ (erb_admin_portal), docs/derived, the editor scaffold
```

Paths in the two legacy documents below are written relative to the **old** repo root. Read `effortless-platform/…` as `rulebook-examples/legacy-runner/…`, and `orchestration/`, `execution-substrates/`, `devops/`, `testing/` as living inside this folder. `rulebook-examples/` and `toy-rulebooks/` are still at the repo root, two levels up.

---

# Part 1 — the original repository README
...
Before ERB, every developer had to teach their LLM the conventions from scratch: how PascalCase tables work, what the Effortless loop is, when to read from `vw_*` views vs base tables, how `effortless build` sequences transpilers. That learning used to cost hours to days per project.
...
| [/effortless-loop](docs/skills/effortless-loop/SKILL.md) | The iterative CHANGE-RULE → REBUILD → CONSUME-VIEWS cycle |
...
# Part 2 — the original `effortless-platform/` README
...
- **init-db** — runs `postgres/reset-rulebook-db.sh` to apply the generated SQL to `erb_admin_portal`.
...
- **clone-skills** — mirrors every `ClaudeSkills` row's `SKILL.md` into `docs/skills/`.
- **effortlessrulebookeditor** — emits `effortless-rulebook/edit-rulebook.sh` + `docker/`, the self-rebuilding browser editor for the platform rulebook (`cd effortless-rulebook && bash edit-rulebook.sh`).
- **rulebook-to-progress-report** — renders `progress-report/progress-report/{progress-report.html, narrative.html, report-summary.md}` from the eight-table progress contract (`UserStories`, `AcceptanceCriteria`, `BuildPhases`, `ERBFeatures`, `ERBFeatureCategories`, `ERBPackages`, `EffortClasses`, `DeliveryDisciplines`).

`rulebook-to-postgres` runs at `[latest]` here, exactly as in every demo — do not pin it. (A stale pin to a decommissioned cpln host silently broke every platform build until 2026-08-30.)

### `progress-report/`

Generated delivery report for the repo-consistency programme. The story corpus, phases, and consistency rules/findings live in the platform rulebook; every number in the report is derived from them. The rollup fields (`BuildPhases.WeightedDonePercent`, `RulebookDomains.ConsistencyGrade`, `ProjectMetadata.ConsistencyPercent`, …) are computed by the rulebook itself, so the report and the `vw_*` views cannot disagree.

> **Tool hosting gotcha.** `rulebook-to-progress-report` is not yet resolvable by bare name from the tools server (its cpln workload is not published as `[latest]`). Run it locally from `Versioned-Stable-SSoTme-Tools/tools/effortless/rulebook-to-progress-report/start.sh` (port **30052**) and point the CLI at it:
> `effortless -setToolUrl rulebook-to-progress-report=http://localhost:30052` — undo with `effortless -removeUrl rulebook-to-progress-report` once it is published.
