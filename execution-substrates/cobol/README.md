...
Formulas like `COUNTIFS(WorkflowSteps!{{Workflow}}, Workflows!{{WorkflowId}})` reference data across tables. The fully-expressive substrates (`Expressive: full` — Postgres, Python, Go, Excel, Entity Framework, …) handle cross-table formulas natively. This is a property of **this COBOL injector** as a target, not of the rulebook or the method: the formula is fully expressible; the open-source COBOL reference injector here simply hasn't been extended to emit the relational scan (see its `Expressive` value in the `ExecutionSubstrates` table).
...
For greenfield projects, prefer any fully-expressive substrate (`Expressive: full`) — e.g. PostgreSQL, Python, or Go.
